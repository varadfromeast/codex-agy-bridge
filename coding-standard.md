...
- **Module-Level Facade pattern**: When exposing a procedural module-level API that delegates to a stateful class instance (e.g., `_orchestrator = RunnerOrchestrator()`), the class implementation and the facade wrappers **MUST** be separated. Move the class to a private/separate module (e.g., `_orchestrator.py`), and define only the delegating wrapper functions in the public entrypoint module (e.g., `orchestration.py`). Re-export classes, variables, and constants cleanly to preserve backwards compatibility.
